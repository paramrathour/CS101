...
This sequence has all rational numbers in range $[0/0\ \text{to}\ 1/1]$ sorted \emph{in increasing order} such that the denominators are less than or equal to $n$ and all numbers are in \emph{reduced forms} i.e., 2/4 does not belong to this sequence as it can be reduced to 1/2.\\
\begin{testcases}
{$n$}
...
Define a function to take a positive integer $n$ as input then repeat the above iteration $n$ times to print the first $2^{n}$ digits\\
\begin{testcases}
{$n$}
